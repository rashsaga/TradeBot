...
class AssetsDatabase:
    ...
    def get_asset_data(self, asset_symbol, key=None):
        ...
        if asset_data is False or key is None:
            return asset_data
        else:
    ...
    def set_asset_data(self, asset_symbol, key, value):
        ...
        value_in_db = self.get_asset_data(asset_symbol, key)
        if value_in_db is False or value_in_db != value:
            self.assetsDB.dadd(asset_symbol, (key, value))
    ...
    def add_asset(self, asset_symbol):
        if self.get_asset_data(asset_symbol) is False:
